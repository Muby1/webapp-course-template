...
async function getAllTasks() {
  // TODO(ch09-1): 手順書 9-5 を参考に、SELECTでtasksテーブルの全件を取得しましょう
  const error = new Error('まだ実装されていません（TODO ch09-1）：手順書 9-5 を参考に、SELECTでtasksテーブルの全件を取得しましょう');
  error.status = 501;
  throw error;
}
...
async function insertTask(title) {
  // TODO(ch09-2): 手順書 9-5 を参考に、プレースホルダー（$1）を使ってtasksテーブルに1件追加しましょう
  const error = new Error('まだ実装されていません（TODO ch09-2）：手順書 9-5 を参考に、プレースホルダー（$1）を使ってtasksテーブルに1件追加しましょう');
  error.status = 501;
  throw error;
}
